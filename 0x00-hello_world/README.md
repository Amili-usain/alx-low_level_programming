# 0x00. C - Hello, World
This project aims at learning compilation using gcc, using the options -Wall -Werror -Wextra -pedantic -std=gnu89
...
* **4. Hello, puts**
  * [4-puts.c](./4-puts.c): C program that prints exactly `"Programming is like building
  a multilingual puzzle`, followed by a new line, using the function `puts`.
    * Use of the `printf` is not allowed.
...
* **5. Hello, printf**			`mandatory`
  * [5-printf.c](./5-printf.c): C program that prints exactly `with proper grammer, but
  the outcome is a piece of art,`, followed by a new line, using the function `printf`.
    * Use of `puts` is not allowed for this task.
...
* **7. Intel**			`#advanced`	
  * [100-intel](./100-intel): A script that generates the assembly code in Intel syntax of a
...
* **8. UNIX is basically a simple operating system, but you have to be a genius to understand the simplicity**			`#advanced`
  * [101-quote.c](./101-quote.c): C program that prints exactly `and that piece of art is
  ...
  without using any functions listed in the NAME section of the man(3) `printf` or man(3)
  `puts`.
